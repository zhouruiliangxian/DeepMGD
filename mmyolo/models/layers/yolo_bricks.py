...
class CSPLayerWithTwoConv(BaseModule):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x_main = self.main_conv(x)
        # print(f"x_main shape main_conv: {x_main.shape}")
        x_main = list(x_main.split((self.mid_channels, self.mid_channels), 1))
        # print(f"x_main shape mid_channels: {x_main.shape}")
        x_main.extend(blocks(x_main[-1]) for blocks in self.blocks)
        # x_z = self.final_conv(torch.cat(x_main, 1))
        # print(f"x_main shape main_conv: {x_z.shape}")
        return self.final_conv(torch.cat(x_main, 1))



# m = UniversalInvertedBottleneckBlock (64, 96, 0, 3, True, 1, 2)
# 96, 96, 3, 0, True, 1, 4

# m = UniversalInvertedResidual(64, 96, dw_kernel_size_start=5, dw_kernel_size_mid=5, stride=2,exp_ratio=4)
# # #uir_r4_a5_k5_s2_e4_c512
# f = f'{m._get_name()}_EXTRA.onnx'
...
#
#         super().__init__()
#         self.uib_convnext = UniversalInvertedResidual(in_chs, out_chs, dw_kernel_size_start=5,dw_kernel_size_mid=0, exp_ratio=4,)
#         self.uib_
#
#
#     def forward(self, x):
...
#         y.extend(m(y[-1]) for m in self.m)
#         return self.cv2(torch.cat(y, 1))
# x = torch.ones(1, 256, 40, 40)
# from timm.models._efficientnet_blocks import MobileAttention
# import os
# m = MobileAttention(256, 256, num_heads=4,)
# # #uir_r1_a5_k0_s1_e4_c512
# f = f'{m._get_name()}.onnx'
# #
# # m = UniversalInvertedResidual(64, 64, dw_kernel_size_start=0, dw_kernel_size_mid=5, stride=1,exp_ratio=4)
# # #uir_r1_a0_k5_s1_e4_c128
# # f = f'{m._get_name()}_IR2.onnx'
#
# # m = UniversalInvertedResidual(64, 64, dw_kernel_size_start=0, dw_kernel_size_mid=0, stride=1,exp_ratio=4)
# # #uir_r2_a0_k0_s1_e4_c256
# # f = f'{m._get_name()}_fnn.onnx'
#
...
#         super().__init__(init_cfg=init_cfg)
#
#         self.c = int(out_chs * e)  # hidden channels
#         self.cv1 = Conv(in_chs, 2 * self.c, 1, 1)
#         self.cv2 = Conv((2 + n) * self.c, out_chs, 1)  # optional act=FReLU(c2)
#         '''
#         'uir_r4_a0_k3_s1_e2_c96',  # IR
#         'uir_r1_a3_k0_s1_e4_c96',  # ConvNeXt
#         '''
#         self.uib_ir = UniversalInvertedResidual(in_chs, out_chs, dw_kernel_size_start=0,dw_kernel_size_mid=3, exp_ratio=2,)
#         self.uib_convnext = UniversalInvertedResidual(in_chs, out_chs, dw_kernel_size_start=5,dw_kernel_size_mid=0, exp_ratio=4,)
#         # 'uir_r4_a0_k3_s1_e2_c96',  # IR
#
#     def forward(self, x: Tensor) -> Tensor:
#         """Forward process."""
#         print(f"Input x shape: {x.shape}")
#         x_main = self.uib_ir(x)
#         print(f"x_main shape after uib_ir: {x_main.shape}")
#         x_main = self.uib_convnext(x_main)
#         print(f"x_main shape after uib_convnext: {x_main.shape}")
#         return x_main
#
# class UIB(BaseModule):
#     """Faster Implementation of CSP Bottleneck with 2 convolutions."""
...
#             [UniversalInvertedResidual(self.c, self.c, dw_kernel_size_start=0,dw_kernel_size_mid=3, exp_ratio=4,),
#             UniversalInvertedResidual(self.c, self.c, dw_kernel_size_start=5,dw_kernel_size_mid=0, exp_ratio=4,)]
#         )
#
#     def forward(self, x):
#         """Forward pass through C2f layer."""
...
# def main():
#     attention_block = UIB_Down(1120,160)
#     input = torch.ones([4, 160, 80, 80])
