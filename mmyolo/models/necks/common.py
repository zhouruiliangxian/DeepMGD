...
class SimFusion_3in(nn.Module):
    def __init__(self, in_channel_list, out_channels,mobile = False):
        super().__init__()
        self.cv1 = SimConv(in_channel_list[0], out_channels, 1, 1)
        if mobile:
            out  = int(out_channels * 2.5)
            self.cv_fuse = SimConv(out, out_channels, 1, 1)
        else:
