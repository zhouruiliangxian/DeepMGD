...
def show_bbox_num(cfg, out_dir, fig_set, class_name, class_num):
    ...
    plt.yticks(fontsize=fig_set['fontsize'])
    # Save figure
...
def show_bbox_wh(out_dir, fig_set, class_bbox_w, class_bbox_h, class_name):
    ...
    # ax.legend(loc='upper left',bbox_to_anchor=(0.2, 1), handles=patches, ncol=2, fontsize=fig_set['fontsize']*1.5)
...
def main():
    ...
    fig_all_set = {
        'figsize': [35, 18],
        'fontsize': int(10 - 0.08 * len(dataset.metainfo['classes']))*3,
        'xticks_angle': 70,
